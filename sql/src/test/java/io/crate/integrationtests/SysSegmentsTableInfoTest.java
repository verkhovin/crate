...
public class SysSegmentsTableInfoTest extends SQLTransportIntegrationTest {
    ...
    public void test_retrieve_segment_information() {
        ...
        execute("select table_name, shard_id, segment_name, generation, num_docs, deleted_docs, size, memory, " +
                "committed, search, compound, version, node, attributes, primary from sys.segments order by primary desc");
    ...
    private void validateResponse(Object[] result, boolean primary) {
        ...
        Map attributes = (Map) result[13];
        assertNotNull(attributes);
        assertThat(attributes.size(), is(1));
        assertThat(result[14], is(primary));
    }
}
